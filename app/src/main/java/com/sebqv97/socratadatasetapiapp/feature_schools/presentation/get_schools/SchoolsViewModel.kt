        ...
        viewModelScope.launch {
            getSchoolsUseCase().collect { collectedResource ->
                when (collectedResource) {
                    is Resource.Loading -> {
                        _getSchoolsState.value = GetSchoolsState(isLoading = true)
                    }
                    is Resource.Error -> {
                        _getSchoolsState.value =
                            GetSchoolsState(encounteredError = collectedResource.errorType)
                    }
                    is Resource.Success -> {
                        _getSchoolsState.value = GetSchoolsState(schools = collectedResource.data!!)
                    }
                }
            }
        }
